...
def get_test_result(test_name, test_num=100):         #获得各类算法的关键字结果，返回一个字典
    ...
    for test_line in test_lines:
        if test_line == 'frequency----textrank----ours-----------------\n':
            ifwrite = True
            num += 1
            if num > test_num:
                break
            ...
            if ifwrite:
                ...
                textRank_keyword.append(line_split[1])
                our_keyword.append(line_split[2].strip())
    test_file.close()
    return freq_dict, textRank_dict, our_dict
...
def acc_test(truth_name, test_name, top_k=10):
    freq_dict, textRank_dict,  our_dict = get_test_result(test_name)
...
def main():
    truth_name = r'..\data\patent_abstract\6种专利摘要各100条已标注\冰箱谢育欣.txt'
    test_name = r'..\data\patent_abstract\6种专利摘要各100条已标注\bingxiang_freq_textRank_ours_techField_wordAVG_1.04_50.txt'
    acc_test(truth_name, test_name, top_k=20)
